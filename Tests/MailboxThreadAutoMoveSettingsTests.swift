...
final class MailboxThreadAutoMoveSettingsTests: XCTestCase {
    ...
    func test_remap_replacesSourceThreadIDsWithGroupedThreadID() {
        let settings = MailboxThreadAutoMoveSettings()
        ...
        settings.remap(threadIDs: ["thread-a", "thread-b"],
                       to: "manual-group",
                       preferredSourceThreadID: "thread-a")

        XCTAssertFalse(settings.rules.contains(where: { $0.threadID == "thread-a" || $0.threadID == "thread-b" }))
        XCTAssertTrue(settings.rules.contains(where: {
            $0.account == "Work" && $0.threadID == "manual-group" && $0.destinationPath == "Projects/Acme"
        }))
        XCTAssertTrue(settings.rules.contains(where: {
