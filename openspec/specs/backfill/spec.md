...
The system SHALL provide Settings actions to run batch operations for the currently selected mailbox over a user-chosen date range (default January 1 of the current year through today on each invocation) and execute ingestion or regeneration off the main actor.
...
#### Scenario: User starts Re-GenAI with defaults
- **WHEN** the user opens Settings and taps "Start Re-GenAI" without changing dates
- **THEN** the date range defaults to January 1 of the current year through today and summary regeneration begins for the currently selected mailbox

#### Scenario: Progress reflects counting and batches
- **WHEN** a batch operation (backfill or Re-GenAI) runs
...
- **WHEN** a batch of 5 fails to import or regenerate
...
- **WHEN** backfill or Re-GenAI finishes
- **THEN** the UI shows success with total processed messages; if failures remain, it shows the error while preserving progress achieved

#### Scenario: UI remains responsive
- **WHEN** backfill or Re-GenAI is running
