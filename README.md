...
- Opt-in Graph view renders the same threaded inbox as a native Obsidian-style force graph: "You" starts at the center and remains draggable, sparse dots and straight links expose folders, threads, and messages, Apple Intelligence generates cached 2–5 word email labels and confirmable ghost groups, and the canvas keeps configurable paging, zoom/pan, pruning, compost restore, keyboard navigation, and persisted preferences.
...
- `GraphCanvasViewModel` maps the existing `ThreadNode` tree, persisted `ThreadFolder` membership, and on-device topic tags into graph-specific group/thread/message nodes. It generates and caches concise on-device graph titles outside the SpriteKit renderer, coordinates graph-only archive state, asks the thread view model to re-scope normal views after archive/restore, and forwards Mail-side snip moves through `MailAppleScriptClient`.
...
- Graph labels use a separate on-device prompt to compress each visible email's completed summary, with its subject as context, into a 2–5 word title. A deterministic safety pass caps even verbose model output at five words and 32 characters while retaining trailing identifiers such as `CR#60`. These titles use a dedicated Core Data cache scope and preserve the full summary for reply-hover/inspector detail. Transient `modelNotReady` availability is retried every 30 seconds for up to ten minutes while Graph is mounted; switching to Timeline cancels the wait or any in-flight generation.
...
- Existing `ThreadFolder` membership renders as solid folder branches. A confirmed folder remains visible whenever at least one of its members is on the current graph page, including singleton folders and partially paged folders; stored thread IDs are whitespace-normalized before matching. Shared tags generated by the existing on-device Apple Intelligence tag provider render as dashed ghost branches; selecting a ghost offers **Create Folder**, which persists a normal `ThreadFolder` and therefore stays consistent with Timeline and folder views. A ghost never changes grouping until the user confirms it.
- To reduce first-render cost, the graph initially maps the persisted Visible branches count (4–24, default 10), caps visible emails per branch, and groups additional roots under a hollow remaining hub that expands by the same configured count per click.
- The graph-controls header reports visible versus total branches; its Filters section searches the graph and changes the persisted visible-branch page size.
- Snip mode is Mail-side: selected thread messages are moved through AppleScript after the user chooses a destination mailbox. The normal path searches the thread's known source mailbox directly instead of crawling every mailbox in Mail, and stops as soon as all requested messages are moved. After a successful snip or archive, the graph plays the branch prune animation before removing the branch from its data model. Archive mode is app-only and persisted in Core Data as graph archive entries.
- The Snip destination sheet honors the preferred parent mailbox when it exists, but falls back to the selected account's complete folder hierarchy when that preference is missing or stale.
- Selecting a graph thread or message reveals a compact triage strip: Action Item toggles the selected email in the existing Action Items store, Not Important opens the whole-thread Snip flow, and Done archives the whole thread in BetterMail. Snip destinations are selected from a native mailbox list and resolved within the chosen account; BetterMail only removes the thread from the graph after Mail confirms that at least one message moved.
...
- The bottom Snip and Archive controls act immediately on a selected thread; with no selection they enter branch-picking mode, where either a branch line, node, or summary callout is a valid target.
